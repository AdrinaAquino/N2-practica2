...
export default function BarraBusqueda({searchValue,setSearchValue,handleClick}) {
    
    const palabraBuscar=(e)=>{setSearchValue(e.target.value)}
console.log(searchValue)
  return (
      <form action="" className='flex justify-center  p-0 mt-8 mb-8' >
        <input type="text" name='search' placeholder='Search...' className=' w-100  p-2 border-3 border-gray-300 border-r-0 rounded-l-md ' value={searchValue} onChange={palabraBuscar}/>
        <button className="border-3 border-gray-300 rounded-r-md"
        onClick={palabraBuscar}>
